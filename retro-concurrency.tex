...
the x86-64 backend on Linux, but our design preclude other backends. The
description in the sequel assumes we are on an x86-64 architecture.
...
introduce stack overflow checks in function prologue of OCaml functions. We
...
stack. Switching between fiber only involves saving the exception and the stack
...
instructions stock OCaml in order to to set up the right stack.
...
aware of fibers layout and how to follow the parent pointers. The stack map
generation and scanning each frame for roots remains the same. Due to the
uniformity of fiber layout, suspended fibers are scanned the same way as the
current active fiber. Previous work~\cite{RetroParallel} presents the
challenges and solution for the interaction of fibers and the concurrent
mark-and-sweep GC of Multicore OCaml.
...
DWARF bytecode at these three places, which has been validated with the DWARF
unwind table validator tool~\cite{Bastian}. When starting to run on a new
fiber, we insert DWARF bytecode to use the follow the |parent_fiber| pointer
