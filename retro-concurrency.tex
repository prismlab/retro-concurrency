...
asynchronous I/O and generators~\ref{WasmPropoal}. Project Loom~\cite{loom} is an
...
semantics~\cite{Felleisen86} for effect handlers, following the abstract machine
semantics of Hillerstrom et al.~\cite{Hillerstrom20}. The crucial difference
...
other research languages with effect handlers~\cite{Hillerstrom20,Frank,Eff}, which
...
coroutines (a non-local control flow mechanism) as a macro-expressible
translation, and present an embedding in Lua and Ruby. Lua provides asymmetric
coroutines~\cite{Lua} where each coroutine uses its own stack similar to how
each handled computation runs in its own fiber in Multicore OCaml. Similar to
Multicore OCaml, Lua does not allow coroutines to include C frames. It is
useful to note that Lua is an interpreted language, unlike OCaml, and DWARF
unwind tables are not a concern in Lua.
...
from exits that would not return. Resource clean up needs to occur only during
...
OCaml does not have a |try/finally| construct available in many programming
languages. In OCaml, alternative standard libraries such as Base~\cite{} and
Core~\cite{} provide mechanisms analogous to Common Lisp |unwind-protect|,
which are in turn implemented using OCaml exceptions. Leijen~\cite{Leijen18}
explicitly extends effect handlers with |finally| clause in Koka. Instead,
Multicore OCaml |discontinue| primitive builds on extant defensive coding
practices against exceptions to clean up resources. This design choice is
crucial to ensure backwards compatibility of legacy systems code with non-local
control flow introduced by effect handlers. Sivaramakrishnan et
al.~\cite{TFP17} describe the interaction of effect handlers and asynchronous
exceptions. This is orthogonal to the contributions of this paper which
focusses on compiler and runtime system support for implementing effect
handlers. Ahman et al.~\cite{Ahman20} present a effect handler core calculus
with asynchrony which might help lay the formal foundations for reconciling
asynchrony in systems and effect handlers.
