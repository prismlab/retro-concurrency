...
\paragraph{Generators} Generators allow data structures to be traversed on
...
\paragraph{Chameneos} Chameneos~\cite{Chameneos} is a concurrency game aimed at
...
\paragraph{Web server} As one of our primary goals of adding effect handlers to
...
\begin{figure}
	...
	\begin{minipage}{0.49\linewidth}
		...
		\label{grf:latency}
	\end{minipage}
	\mycaption{Web server performance.}
	\vspace{-4mm}
\end{figure}
...
continuation passing style. Leijen et al.~\cite{Leijen14} implements effect
...
into a context. |dynamic-wind| is known to be complex and not quite what one
wants for resource clean up~\cite{Kiselyov,Sitaram03}. The main challenge is distinguishing
non-returning exits from ones that would, and clean up to occur only on
non-returning exit. In Multicore OCaml, we assume that continuations are used
linearly, and hence, no clean up occurs when effects are performed but only when
an exception is thrown, perhaps from the use of |discontinue|. This obviates
the need for |dynamic-wind|. Leijen~\cite{Leijen18} explicitly extends effect
handlers with |finally| clause. Instead, Multicore OCaml |discontinue|
primitive builds on extant defensive coding practices against exceptions to
clean up resource.
...
return more than once, leading to errors such as double-freeing malloc'ed
memory. Since our goal is to retrofit effect handlers onto OCaml, we relegate
continuation cloning to the OCaml |Obj| module, which contains unsafe features.
