...
form of delimited continuations~\cite{Danvy90}, which enables advanced
non-local control-flow mechanisms such as resumable exceptions, coroutines,
generators and asynchronous I/O to be composably expressed. As an example,
consider the task of handling division-by-zero case in a calculator
application. Different calculators handle division-by-zero differently. In the
iOS 13 Calculator App (|iOS|), division-by-zero returns |Error|. In the Google
search calculator App (|gsearch|) |0/0| returns to |Error|, and for any other
|n| not equal to |0|, |n/0| returns |Infinity| with the same sign as |n|. How
can we capture these different behaviours without changing the evaluator?
...
handlers~\cite{Leijen14,Hillerstrom20,Pyro,Frank,Eff}. Unlike these efforts, our goal is to
...
tools~\cite{everest,Coq}, mission critical software systems~\cite{astree} and
latency sensitive networked applications~\cite{Madhavapeddy13}. OCaml is
...
using callbacks and (2) efficient nested parallel programs in the vein of
Cilk~\cite{Cilk}, OpenMP~\cite{OpenMP}, X10~\cite{Charles05}, Intel
TBB~\cite{IntelTBB}, and variants of Parallel ML~\cite{MaPLe, Fluet10,
Sivaramakrishnan14}.
...
representation (IR)~\cite{Leijen14}. If the compiler were to use CPS, then reifying
...
get allocated on the heap~\cite{Farvardin20}. Moreover, with CPS, stack
unwinding is meaningless and hence, we would lose compatibility with tools that
inspect the program stack. Hence, we choose not to use CPS translation and
represent continuations as call stacks.
...
Our work is also timely. WebAssembly~\cite{Hass17} community group is actively
considering effect handlers as one of the mechanism for supporting concurrency,
asynchronous I/O and generators~\cite{WasmPropoal}. Project Loom~\cite{loom} is an
OpenJDK project which adds virtual threads and delimited continuations to Java.
Swift concurrency roadmap~\cite{swift} includes direct-style asynchronous
...
\begin{itemize}
  \item the design and implementation of one-shot effect handlers for OCaml.
    ...
    unwind tables using an automated validator tool~\cite{Bastian19}.
...
managing the program stack. In this section, we provide an overview of the
program stack and related mechanisms in stock OCaml.
...
for bump pointer allocation, and |r14| holds a reference to the |Caml_state|
...
OCaml heap, then the external functions can be called directly, and no
bookkeeping is necessary. For external functions which allocate in the OCaml
heap, the cached allocation pointer is saved to a field in the |Caml_state|
before the external call, and loaded back on return. Similarly, callbacks into
OCaml are also cheap. Callbacks into OCaml involves loading the arguments in
the right registers and calling the OCaml function. OCaml callbacks are
relatively common as the garbage collector (GC) finalisation functions are
executed as OCaml callbacks.
...
\end{lstlisting}
The function |input_line| raises |End_of_file| exception on reaching the end of
...
\begin{figure*}
	\begin{smathpar}
		...
		\end{array}
	\end{smathpar}
	\mycaption{Top-level reductions}
	\label{sem:toplevel}
	\vspace{-3mm}
\end{figure*}
\begin{figure*}
...
$\clos{x}{e}{\env}$, effect being performed $\effval{l}{k}$ and exception being
raised $\exnval{l}$. The environment is a map from variables to values.
...
that directly using |mimalloc|~\cite{Leijen19} outperforms having the stack
cache.
...
unwind table validator tool~\cite{Bastian19}. When starting to run on a new
...
which allows compiling pure code in direct-style and effectful code in
continuation passing style. Leijen et al.~\cite{Leijen14} implement effect
handlers as a library in C using stack copying. Since C allows pointers into
the stack, care is taken to ensure that when continuations are resumed, the
constituent frames are restored to the same memory addresses as at the time of
capture. Kiselyov et al.~\cite{Kiselyov18} use an implementation of
multi-prompt delimited continuations as an OCaml library~\cite{Kiselyov12} to
embed Eff language in OCaml. Indeed, Forester et al.~\cite{Forster19} showed
that in an untyped setting, effect handlers, monadic reflection and delimited
control can macro-express each other.

Multicore OCaml effect handlers are similar to~\cite{Leijen14, Kiselyov12}, but
are more efficient; since we only allow our continuations to be resumed at-most
once, there is no need for copying frames. Bruggeman et al.~\cite{Bruggeman96}
show how to implement one-shot continuations efficiently using segmented stacks
in Scheme. Farvardin et al.~\cite{Farvardin20} perform a comprehensive
evaluation of various implementation strategies for continuations on modern
hardware. Multicore OCaml stacks do not neatly fit the description of one of
these implementation strategies. They best described as using the resize
...
The interaction on non-local control flow and resources have been studied
extensively. Scheme uses |dynamic-wind| to ensure de-allocation (and
re-allocation) of resources when the non-local control leaves and enters back
into a context. |dynamic-wind| is considered to be complex and not quite what
one wants for resource clean up~\cite{Kiselyov,Sitaram03}. The main challenge
is distinguishing returning exits from a computation closing over resources
from non-returning ones. Resource clean up needs to occur only during
non-returning exit. In Multicore OCaml, we assume that continuations are used
linearly, and hence, no clean up occurs when effects are performed but only
when an exception is thrown, perhaps from the use of |discontinue|. This
obviates the need for |dynamic-wind|.

OCaml does not have a \texttt{\footnotesize try/finally} construct available in
many programming languages. In OCaml, alternative standard libraries such as
Base~\cite{} and Core~\cite{} provide mechanisms analogous to Common Lisp
|unwind-protect|, which are in turn implemented using OCaml exceptions.
Leijen~\cite{Leijen18} explicitly extends effect handlers with |finally| clause
in Koka. Instead, Multicore OCaml |discontinue| primitive builds on extant
defensive coding practices against exceptions to clean up resources. This
design choice is crucial to ensure backwards compatibility of legacy systems
code with non-local control flow introduced by effect handlers.
...
of this paper which focusses on compiler and runtime system support for
implementing effect handlers. Ahman et al.~\cite{Ahman20} present a effect
handler core calculus with asynchrony which might help lay the formal
foundations for reconciling asynchrony in systems and effect handlers.
...
programming~\cite{Pyro}. Multicore OCaml has an experimental feature for
explicitly cloning continuations, which makes a copy of each of its constituent
fibers. Multi-shot continuations are difficult to reason about in the presence
of linear resources such as file descriptors. Recall that OCaml programs are
written with the expectation that function calls return \emph{exactly-once}
either normally or exceptionally. With multi-shot continuations, they may
return \emph{more than once}, leading to errors such as double-freeing
malloc'ed memory. Since our goal is to retrofit effect handlers onto OCaml, we
relegate continuation cloning to the OCaml |Obj| module, which contains unsafe
features.
