...
speeding up fiber allocation~\cite{Farvardin20} as we observed in our
evaluation (\S\ref{sec:eval}) that directly using |mimalloc|~\cite{Leijen19}
outperforms a stack cache.
...
The biggest impact was the increase in the code size due to stack overflow
checks. We define code size as the sum of the sizes of all the OCaml text
sections in the compiled binary file. The code size metric excludes data, debug
symbols, the runtime (implemented in C) and other C libraries. Compared to
|stock|, the code size for OCaml code is 19\% more for |MC| and |MC+RedZone32|,
and 30\% more for |MC+RedZone0|. The result shows that our 16-word red zone is
effective at reducing the code size compared to having no red zone, whereas the
32-word red zone does not noticeably reduce the code size further. Further work
is required to bring the code size closer to |stock|.
...
resource safety. Sivaramakrishnan et al.~\cite{TFP17} describe the interaction
...
handlers. We define code size as the sum of the sizes of all the text sections
in the binary generated by compiling the OCaml code. Thus, the code size metric
excludes data, debug symbols, the runtime (implemented in C) and other
statically linked C libraries. The |MC| is Multicore OCaml with the default red
zone size (16 words). |MC+RedZone0| uses red zone size of 0; every OCaml
function has a stack overflow check. |MC+RedZone32| uses a red zone size of 32
words.
