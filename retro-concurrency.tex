...
\newcommand{\caseexn}[3]{\kw{exception} ~#1 ~#2 \allowbreak \mapsto #3}
...
This continuation may be used to resume the suspended computation later. This
enables non-local control-flow mechanisms such as resumable exceptions,
lightweight threads, coroutines, generators and asynchronous I/O to be
...
handlers~\cite{Leijen14,Biernacki20,Hillerstrom20,Pyro,Frank,Eff}. Unlike these
efforts, our goal is to retrofit effect handlers onto the OCaml programming
language, which has been in continuous use for the past 25 years in large
codebases including verification tools~\cite{everest,Coq}, mission critical
software systems~\cite{astree} and latency sensitive networked
...
perform stack overflow checks, and external C functions (which do not have
stack overflow checks) must be performed on a separate system stack.
Additionally, we must generate DWARF stack unwind tables for stacks that may be
non-contiguous. In this work, we develop the compiler and runtime support
required for implementing efficient effect handlers for OCaml that satisfy
these requirements.
...
\begin{itemize}
	...
	\item a formal operational semantics for the effect handler implementation in
		OCaml. Our formalism explicitly models the interactions with the C stack,
		which is generally overlooked by other formal models, but which the
...
let _ = assert (omain () = 42)
  ...
  \end{minipage}
  \begin{minipage}{\linewidth}
		\begin{lstlisting}[language=c, xleftmargin=0.05\linewidth]
...
\begin{lstlisting}[numbers=left, xleftmargin=0.025\textwidth]
let run main =
...
\begin{lstlisting}[numbers=left, xleftmargin=.025\textwidth]
let run main =
  let runq = Queue.create () in
  ...
  let rec run_next () =
    ...
    | exception Queue.Empty ->
      ...
      | todo ->
        ...
        List.iter (fun (s,k) ->
         Queue.push (fun () -> continue k s) runq) compl;
        pending_reads := todo;
        run_next ()
  in
  let rec spawn f =
...
effect safety. Programs without matching effect handlers are well-typed
Multicore OCaml programs. As a result, our static semantics is simpler than
languages that ensure effect safety~\cite{Eff, Hillerstrom20, Leijen14, Effekt,
Frank, Biernacki19}. This is important for backwards compatibility as our goal
is to retrofit effect handlers to a language with large legacy codebases;
programs that do not use effects remain well-typed, and those that do compose
...
For effect handling, we extend OCaml's |match ... with| syntax with effect
...
OCaml in Multicore OCaml to ensure performance backwards compatibility. This
differs from other research languages with effect
...
design choice in stock OCaml has a significant impact in enabling fast
switching between fibers in Multicore OCaml.
...
represent the table using a compact bytecode representation that describes the
unwind table as a sequence of edits from the start of the function. In order to
compute the call-frame information at any given instruction within a function,
the DWARF bytecode from the start of that function must be interpreted on
demand. For each function, DWARF maintains a \emph{canonical frame address}
(CFA) and is traditionally the stack pointer before entering this function.
Hence, on x86-64, where the return address is pushed on the stack on call, the
return address is at |CFA - 8|.

Our goal is to compute the CFA of the caller when stacks are switched using the
DWARF directives. Recall that stack switching occurs in effect handlers,
external calls and callbacks. At the entry to an effect handler block, we insert
DWARF bytecode to follow the |parent_fiber| pointer and dereference the
...
handlers. We have verified the correctness of our DWARF directives using the
verification tool from Bastian et al.~\cite{Bastian19}.
...
call stack, which would capture the complete backtrace rather than just the
current fiber.
...
The challenges and the solutions for integrating fibers with the concurrent
...
an extension of the OCaml 4.10.0 compiler with support for shared memory
...
\begin{figure*}
	\includegraphics[scale=0.34]{sandmark-notebook/sandmark_time}
	\vspace{-4mm}
...
\begin{figure*}
	\includegraphics[scale=0.34]{sandmark-notebook/sandmark_codesize}
	\vspace{-4mm}
...
Lwt~\cite{lwt} and Async~\cite[\S18]{rwo}) also have other downsides -- exceptions,
