...
Cilk~\cite{Cilk}, OpenMP~\cite{}, Habenero Java~\cite{Imam14}, X10~\cite{},
Intel TBB~\cite{IntelTBB}, parallel Haskell variants~\cite{} and parallel ML
variants~\cite{}.
...
benchmarks implemented using concurrency monad~\cite{} (|monad|) as a proxy for
CPS versions. Recall that OCaml compiler does not use CPS in its IR. In the
|monad| version, we use a |fork| to invoke the non-tail call and use an |MVar|
...
Our concurrency monad, and indeed monadic concurrency libraries such as Lwt and
Async, also has other downsides; due to the lack of stack, exceptions,
backtraces, and DWARF unwinding are no longer useful.
...
a hand-written, selective CPSed~\cite{},
defunctionalised~\cite{DefunctionalizationAtWork} version (|cps|) and a
...
\paragraph{Chameneos} Chameneos~\cite{} is a concurrency game aimed at
measuring context switching and synchronization overheads. Our implementation
uses MVars~\cite{} for synchronization. We compare effect handler (|MC|),
concurrency monad (|monad|) and Lwt, a widely used concurrency programming
library for OCaml (|lwt|) versions. We observed that |MC| was the fastest, and
|monad| (|lwt|) was 1.67$\times$ (4.29$\times$) slower than |MC|.
...
use \texttt{httpaf} library~\cite{} for HTTP handling, and
\texttt{libev}~\cite{} for the eventloop. We compare our implementation against
an Lwt version (|lwt|) which also uses \texttt{httpaf} and \texttt{libev}.
Unlike using effect handlers, Lwt version is written in monadic style, and does
not have a notion of thread per request. For comparison, we also include a Go
version (|go|) that uses \texttt{net/http}~\cite{} package.
...
The client workload was generated with \texttt{wrk2}~\cite{}. We maintain 1k
open connections and perform requests for a static web page at different rates,
and record the request service rate and latency. The throughput and tail
...
We measure the tail latencies at 2/3rd of this rate (at 20k requests per
second), as would be the case in real deployments. We observe that both the
OCaml versions remain competitive with |go|, and |MC| performs best in terms of
tail latency.
...
Such a feature is provided by Go~\cite{}, but not possible with OCaml
