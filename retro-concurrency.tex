...
Many programming languages, including OCaml, require non-local changes to source
code in order to support asynchronous I/O, often leading to a dichotomy between
synchronous and asynchronous code~\cite{colour}.
Since OCaml only provides preemptive system threads as support for concurrency,
users usually use cooperative I/O libraries such as Lwt~\cite{lwt} and
Async~\cite{async}, where all asynchronous code must
use a monadic style. Languages such as GHC Haskell and Go provide lightweight
...
directly in OCaml as a library without imposing a monadic form on users of it.
