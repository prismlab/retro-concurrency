...
\end{minipage}
  \begin{minipage}{0.39\linewidth}
    ...
    \subcaption{Stack layout before raising exception \texttt{E1}.}
...
value of type |'b|. The last case in |effc| forwards any unmatched effect to
the outer handler and returns the value and exceptions back to the original
performer.
...
We have included an executable version of the dynamic semantics as an OCaml
program in the supplementary material \kc{include hl-semantics.ml}.
...
either an empty stack $\ostackemp$ or non-empty $\ostacka{k}{\cstack}$ with the
...
\textsc{RetToC} returns a value to the enclosing C stack. In this case, we have exactly one
...
\kc{Include example reductions from hl-semantics.ml in Appendix}
...
the x86-64 backend on Linux, but our design doesn't preclude other backends.
The description that follows assumes a x86-64 architecture.
...
\begin{figure}
  \includegraphics[width=0.7\linewidth]{figures/fiber}
  \caption{Layout of Multicore OCaml fiber}
  \label{fig:fiber}
\end{figure}
...
case, continuations will be resumed at-most once, and copying fibers is
unnecessary and inefficient. Instead, Multicore OCaml allocates fibers on the
heap. Figure~\ref{fig:fiber} shows the layout of a suspended fiber in Multicore
OCaml. A fiber is said to be suspended if it is not the current (top-most)
fiber during OCaml execution.
...
that invokes value handler after switching to the parent stack. The stack is
laid out such that when the computation in the handler returns, control goes to
value handler.
...
When the stack pointer |rsp| goes less than stack threshold (a field in the
...
introduce stack overflow checks into the function prologue of OCaml functions. We
observed that most function calls are to leaf functions with small frame size.
In order to take advantage of this fact, we introduce a small red zone at the
top of the stack. The compiler elides the stack overflow check for leaf
...
stack. Switching between fibers only involves saving the exception and the stack
pointer of the current stack and loading the same on the target stack. Since
OCaml does not generate pointers into the stack, the two |fiber_info| fields
are the only ones that need to be updated when fibers are moved. We allocate
the space for the fiber using |malloc| and |free| it when no longer required.
We do not use a \emph{stack cache} of recently freed stacks for speeding up
fiber allocation. In our experiments, we observed that directly using
|mimalloc| outperforms having the stack cache.
...
saving the exception pointer and the stack pointer in the |fiber_info| of the
current fiber, and the allocation pointer in the |Caml_state|, updating |rsp|
to the top of system stack (maintained in |Caml_state|), and calling the C
function. The actions are reversed when returning from the external call. One
subtlety is that for C functions that take arguments on the stack, the
arguments must be copied to the C stack from the OCaml stack where they are
laid out. For vast majority of external calls, we perform only a few additional
instructions over stock OCaml in order to to set up the right stack.

\begin{figure}
  \includegraphics[width=0.69\linewidth]{figures/multicore_stack}
	\caption{Layout of Multicore OCaml stack on \texttt{meander} example from
	\S\ref{sec:stack}.}
  \label{fig:mcstack}
\end{figure}
...
\begin{figure}
	\includegraphics[width=0.6\linewidth]{figures/fiber_handler}
	\caption{Layout of stack with effect handlers}
...
effect F : unit;;
match (* comp_e *)
  match (* comp_f *) (*p1*) perform E (*p3*)
  with v -> v | effect F kf -> continue kf ()
with v -> v | effect E ke -> (*p2*) continue ke ()
\end{lstlisting}
The code performs |E| which is handled in the outer-most handler, and is
immediately resumed. The layout of the stack as the program executes is
captured in Figure~\ref{fig:fiber_handler}. The arrows between rectangular
fibers are parent pointers. At position |p1|, |rsp| is at the top of the fiber
|f|.

When an effect is performed, we invoke the effect closure |clos_heff| in the
fiber |f| with the current fiber pointer as the continuation. Since the first
handler does not handle effect |E|, the effect is forwarded to outer handler by
performing the effect closure of fiber |e|. This handles |E|. We allocate a one
word continuation object |ke| on the heap that holds the pointer to fiber |e|
which points to the fiber |f| (position |p2|). Thus, continuations are captured
without copying frames. Since each handler closure is evaluated until a machine
one is found, the take to handle an effect is linear in the number of handlers.

\tk{Something wasn't quite right with this: we allocate the continuation object
at the point of the perform. Also should we describe how the first handler
passes the effect on and adds the e onto the continuation that started with f?}
...
aware of fibers layout, the closures for the handlers and how to follow the
parent pointers. The stack map generation and scanning each frame for roots
remains the same. Due to the uniformity of fiber layout, suspended fibers are
scanned the same way as the current active fiber. Previous
work~\cite{RetroParallel} presents the challenges and solution for the
interaction of fibers and the concurrent mark-and-sweep GC of Multicore OCaml.
...
building the unwind table. Within each function, DWARF maintains a
...
fiber, we insert DWARF bytecode to follow the |parent_fiber| pointer
and dereference the |saved_sp| to get the CFA. During callback, we save the
current system stack pointer in the DWARF block in Figure~\ref{fig:fiber} to
identify the CFA in the C stack. DWARF unwinding for external calls is
implemented similarly by saving the current OCaml stack pointer. With these
changes, we get the same backtrace for the |meander.exe| program from
\S\ref{sec:unwind}, modulo runtime system functions due to effect handlers.
