...
effect handlers is particularly attractive as they allow advanced control flow
mechanisms such as first-class continuations, generators, async/await,
coroutines to be composably expressed. This paper presents a design, a
full-fledged implementation and an evaluation of effect handlers for OCaml.
OCaml is industrial-strength multi-paradigm programming language that tends to
be widely used for systems programming, our implementation strives to maintain
the existing performance profile and toolchain compatibility. Retrofitting
effect handlers onto OCaml is challenging since OCaml does not have any
non-local control flow mechanisms other than exceptions. Our implementation of
effect handlers for OCaml imposes negligible overhead on code that does not use
effect handlers, remains compatible with program analysis tools that inspect the
stack and is efficient for code that makes use of effect handlers.
\end{abstract}
...
On line 4, we declare an effect |DivBy0| that is parameterised by an integer,
which when performed returns the result of division-by-zero. If the denominator
is 0, then we perform |DivBy0| effect with the numerator value (line 12) so that
the context may decide the return value. We can get the |iOS| semantics by
wrapping the evaluator with the following handler:
\begin{lstlisting}
...
other hand, we can get the |gsearch| semantics by enclosing the evaluator in the
following handler:
\begin{lstlisting}
...
callee-saved registers in OCaml. On the |x86-64| backend, the OCaml runtime
makes use of C callee-saved registers: |r15| holds the \emph{allocation pointer}
into the minor heap used for bump pointer allocation, and |r14| holds a
reference to the |Caml_state| table of global variables used by the runtime.
This makes external calls extremely fast in OCaml. If the external function does
not allocate in the OCaml heap, then it can be called directly and no
bookkeeping is necessary. For external functions which allocate in the OCaml
heap, the cached allocation pointer is saved to |Caml_state| before the external
call and it is restored on return. Similarly, callbacks into OCaml are also
cheap: these involve loading the arguments in the right registers and calling
the OCaml function. OCaml callbacks are relatively common as the garbage
collector (GC) finalisation functions are executed as OCaml callbacks.
...
handler and the current exception pointer (|exn_ptr| -- a field in |Caml_state|)
to the stack. At this point, the current exception pointer is updated to be the
...
stack for assisting the GC. For every call point in the program, the OCaml compiler
...
speeding up fiber allocation~\cite{Farvardin20}. In our experiments, we observed
...
extensively. Scheme uses |dynamic-wind|~\cite{}, which is a generalisation of
Common Lisp |unwind-protect|~\cite{}, which ensures de-allocation (and
re-allocation) of resources every time the non-local control leaves and enters
back into a context. |dynamic-wind| is considered to be complex and not quite
what one wants for resource clean up~\cite{Kiselyov,Sitaram03}. The main
challenge is distinguishing returning exits from non-returning ones. Resource
clean up needs to occur only during non-returning exit.
...
ensures backwards compatibility of legacy OCaml systems code with non-local
control flow introduced by effect handlers.

Leijen~\cite{Leijen18} explicitly extends effect handlers with |initially| and
|finally| clauses in Koka for resource safety. Sivaramakrishnan et
al.~\cite{TFP17} describe the interaction of effect handlers and asynchronous
exceptions. This is orthogonal to the contributions of this paper. Our focus i
the compiler and runtime system support for implementing effect handlers.
...
that use effect handlers. Our evaluation shows that this is indeed the case. We
plan to upstream the compiler and runtime support for effect handlers followed
by an effect system.
