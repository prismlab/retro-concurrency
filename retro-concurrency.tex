...
\newcommand{\heap}{\Delta}
\newcommand{\heapext}[2]{\heap[#1 \mapsto #2]}
...
\renewcommand{\configa}[4]{\|#1,#2,#3,#4\|}
...
\begin{figure*}
	...
	\begin{smathpar}
	\begin{array}{rrcl}
		\text{Handler Closures} & \hc & \coloneqq & (h,\env,k) \\
		\text{Frame List} & \fl & \coloneqq & [] \mid r :: \fl \\
		\text{Fibers} & \fiber & \coloneqq & (\fl, \hc) \\
		\text{Heap} & \heap & \coloneqq & \emptyset \mid \heapext{k}{\fiber} \\
		\text{C stacks} & \cstack & \coloneqq & \cstacka{\fl}{\ostack}\\
		\text{OCaml stacks} & \ostack & \coloneqq & \ostacka{k}{\cstack} \mid \ostackemp \\
		\text{Stacks} & \stack & \coloneqq & \cstack \mid \ostack \\
		\text{Terms} & \term & \coloneqq & e \mid v \\
		\text{Configurations} & \config & \coloneqq & \configa{\tau}{\env}{\stack}{\heap}
	\end{array}
	\end{smathpar}
	\end{minipage}
	\caption{Term Syntax}
...
\begin{figure*}
	\begin{smathpar}
		\begin{array}{rrcll}
			\textsc{LocalO}   & \configa{\term}{\env}{\ostacka{k}{\cstack}}{\heapext{k}{(\fl,\hc)}} & \ostep
												& \configa{\term'}{\env'}{\ostacka{(k,\hc)}{\cstack}}{\heapext{k}{(\fl',\hc)}}
												& \text{if } (\term,\env,\fl) \rightsquigarrow (\term',\env',\fl') \\
			\textsc{RetC}     & \configa{v}{\_}{\ostacka{k}{\cstack}}{\heap[k \mapsto ([],(h,\env,k'))]} & \ostep
												& \configa{e}{\envext{\env}{x}{v}}{\cstack}{\heap}
												& \text{if } \{\caseval{x}{e}\} \in h \\
									& & & & \text{and } k' \notin \heap \\
			\textsc{RetFib}   & \configa{v}{\_}{\ostacka{k}{\cstack}}{\heap[k \mapsto ([],(h,\env,k'))][k' \mapsto \fiber]} & \ostep
												& \configa{e}{\envext{\env}{x}{v}}{\ostacka{k'}{\cstack}}{\heapext{k'}{\fiber}}
												& \text{if } \{\caseval{x}{e}\} \in h \\
			\textsc{CallO}    & \configa{v}{\_}{\ostacka{k}{\cstack}}{\heapext{k}{(\oclos{x}{e}{\env}::\fl,\hc)}} & \ostep
												& \configa{e}{\envext{\env}{x}{v}}{\ostacka{k}{\cstack}}{\heapext{k}{(\fl,\hc)}}\\
			\textsc{ExtCall}  & \configa{v}{\_}{\ostacka{k}{\cstack}}{\heapext{k}{(\cclos{x}{e}{\env}::\fl,\hc)}}  & \ostep
												& \configa{e}{\envext{\env}{x}{v}}{\cstacka{[]}{\ostacka{k}{\cstack}}}{\heapext{k}{(\fl,\hc)}} \\
			\textsc{Taken}    & \configa{v}{\_}{\ostacka{k}{\cstack}}{\heap[k \mapsto (\ffun{k'}::\ffun{\oclos{x}{e}{\env}}::\fl,\hc)]} & \ostep
												& \configa{e}{\emptyset}{\ostacka{k}{\cstack}}{\heapext{k}{(\fl,\hc)}}
												& \text{if } k' \notin \heap \\
									& & & & \text{and } e = \throw{\textsf{taken}}{0} \\
			\textsc{Resume}   & (v, \_, (\ffun{k}::\ffun{\oclos{x}{e}{\env}}::\fl,\hc) \kcons k', \cstack) & \ostep
												& \configa{e}{\envext{\env}{x}{v}}{\ostacka{k ~@~ (\fl,\hc) \kcons k'}{\cstack}} \\
			\textsc{Handle}   & (\handle{e}{h}, \env, k, \cstack) & \ostep
												& \configa{e}{\env}{\ostacka{([],(h,\env)) \kcons k}{\cstack}} \\
